...
Class Cdnmd5Rsc
{
    ...
    const RACKSPACE_US = 'https://identity.api.rackspacecloud.com/v2.0/';
    /**
    ...
    private function container($containerName = null)
    {
        ...
        $connection = new \OpenCloud\Rackspace(
            $auth['authurl'],
            ['username' => $auth['username'], 'apiKey' => $auth['key']]
        );
    ...
    {
        $list = $container->ObjectList(['prefix' => $filename]);
        $files = [];
        while ($o = $list->Next()) {
            $files[] = $o->name;
        }
     ...
     */
    public static function purge($olderThan = '-3 months', $excludeHashes = [], $config = [])
    {
        $_this = Cdnmd5Rsc::getInstance($config);
        $container = $_this->container();
        ...
        while ($o = $list->Next()) {
            ...
            if (strtotime($o->last_modified) > $olderThanEpoch) {
                $_this->log[] = "purge skipped: age " .
                    date('c', strtotime($o->last_modified)) . " > " .
                    date('c', $olderThanEpoch) . " for " . $o->name;
                continue;
            }
            if ($o->Delete()) {
                $files[] = $o->name;
